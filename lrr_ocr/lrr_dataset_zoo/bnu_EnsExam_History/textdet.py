data_root = '/lirunrui/datasets/mmocr_bnu_EnsExam_History'
...
test_preparer = dict(
    obtainer=dict(
        ...
        files=[
            dict(
                url='https://rrc.cvc.uab.es/downloads/ch4_training_images.zip',   # 随便写的不起作用
                save_name='bnu_EnsExam_History',   # 这个写成本来是SCUT_HCCDoc_Dataset的，然后解压的时候操作会变成将`cache_path/save_name`复制到`data_root`中。
                md5=None,
                is_extract = False, 
                content=['image','annotation'],
