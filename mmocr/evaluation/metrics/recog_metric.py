...
@METRICS.register_module()
class CRandARMetric(BaseMetric):
    """ correct rate (CR), accuracy rate (AR) for text recognition task.
    ...
    def process(self, data_batch: Sequence[Dict],
        ...
        for data_sample in data_samples:
            ...
            result = dict(
                gt_text_lower_ignore = gt_text_lower_ignore,
                pred_text_lower_ignore = pred_text_lower_ignore,
            )
            self.results.append(result)
    ...
    def compute_metrics(self, results: Sequence[Dict]) -> Dict:
        ...
        for result in results:
            line_len = len(result['gt_text_lower_ignore'])
            if line_len == 0: continue

            dis, error, _, _, _ = self._cal_distance(result['gt_text_lower_ignore'],result['pred_text_lower_ignore'])

            AR = (line_len - error[0] - error[1] - error[2]) / line_len
            CR = (line_len - error[0] - error[1]) / line_len
            AR_list.append(AR)
            CR_list.append(CR)

        CR = sum(CR_list)/len(CR_list)
        AR = sum(AR_list)/len(AR_list)
    ...
    def _cal_distance(self, label_list, pre_list):
        y = SequenceMatcher(a = label_list, b = pre_list)
        yy = y.get_opcodes()
        insert = 0
        delete = 0
        replace = 0
