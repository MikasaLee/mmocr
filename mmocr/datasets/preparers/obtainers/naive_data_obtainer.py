...
class NaiveDataObtainer:
    ...
    def __call__(self):
        for file in self.files:
            ...
            is_extract = file.get('is_extract', True)  # new
            if is_extract:  # new
                self.extract(src_path=download_path, dst_path=self.data_root)  # new
            # self.extract(src_path=download_path, dst_path=self.data_root) # old
            ...
            try:
                if osp.isfile(src_path):
                    ...
                    if os.path.exists(dst_path):
                        shutil.rmtree(dst_path)    # dst_path 全部移除，不管空不空
                    shutil.copytree(src_path, dst_path)
            except:
                print('Warning: copy is not success!')
                raise Exception
            return
